...
======== ==================================================================
``t``    帧类型: ``cloud`` / ``imu`` / ``slam`` / ``grid`` / ``status`` / ``log`` / ``cmd``
``k``    来源键: ``fused`` / ``front`` / ``rear`` / ``lidar_front`` / ``lowstate`` ...
``ts``   采集端墙钟时间（float 秒，time.time()），ROS2 侧据此打 header.stamp
======== ==================================================================

**反向（node → 采集器）只有一种帧：控制指令 ``t="cmd"``** —— 它是全桥唯一的下行通道，
只在 ``motion.enabled=true`` 时才有内容（见 ``motion.py``）：

====================== ==========================================================
``{"k":"move", vx,vy,vyaw}`` 速度目标 → 采集器按 ``motion.rate_hz`` 调 ``sport_client.Move()``
``{"k":"stop", reason}``    立刻刹车 → ``sport_client.StopMove()``（cmd_vel 超时/上游收工）
====================== ==========================================================

两个方向独立：采集器在同一 socket 上只写不读、ROS2 节点只读不写，互不干扰（TCP 全双工）。
"""
